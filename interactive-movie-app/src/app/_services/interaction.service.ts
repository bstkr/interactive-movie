...
import { InteractionState, InteractionStateArray } from '../_models/InteractionState';
...
export class InteractionService {
  ...
  getInteractionState(interactionName: string) {
    return this.interactionStateObservableArray.find( interactionObs => 
      interactionObs.interactionName === interactionName);
  }

  isInteractionClicked(interactionName: string) {
    return this.interactionStateObservableArray.find( interactionObs => 
      interactionObs.interactionName === interactionName).clicked;
  }
  ...
  getPathToImageForClickedInteraction(interactionName: string, interactionId: string, decision: string):string {
    if (decision === 'a') {
      return this.interactionStateArray.find(interactionState => 
        interactionState.name === interactionName).Interactions.find(interaction =>
          interaction.interactionId === interactionId).pathToCompleteObjectImage.a;
    } else if (decision === 'b') {
      return this.interactionStateArray.find(interactionState => 
        interactionState.name === interactionName).Interactions.find(interaction =>
          interaction.interactionId === interactionId).pathToCompleteObjectImage.b;
    } else {
      return '';
    }
  }
  ...
  private initializeObservableArray() {
    for (let interactionState of this.interactionStateArray) {
      let arrayObject = new InteractionObservable(interactionState.name, interactionState.clicked, interactionState.decision);
  ...
  private getDecisionOfInteractionState(interactionName: string): string {
    return this.interactionStateArray.find(interactionState => 
      interactionState.name === interactionName).decision;
  }
